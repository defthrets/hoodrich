...
namespace Hoodrich.Supply
{
    ...
    internal sealed class SupplierManager
    {
        ...
        /// Built-in contacts.
        ///
        /// The docks are the one-stop shop: a worker there can get you ANY product (empty Drugs
        /// list) at a flat price, so you always have a way to restock. Every gang contact sells
        /// only what that gang actually moves, but sells it cheap -- which is what makes it worth
        /// crossing the map, and worth staying on good terms with people.
        /// </summary>
        private void AddDefaults()
        {
            _defs.Add(new SupplierDef
            {
                ...
                OpenHour = 5,
                CloseHour = 21,
                Blurb = "Gets anything off the containers. Daylight only.",
                Models = { "s_m_y_dockwork_01", "s_m_m_dockwork_01", "s_m_y_construct_01", "s_m_m_linecook" }
                // Drugs deliberately empty: the docks carry the whole catalogue.
            });

            AddGangContact("armenians", "Armenian Connection", "ARM", "heroin", 0.70f, 2, 150f,
                "Heroin, straight off the boat.",
                new[] { "g_m_m_armboss_01", "g_m_m_armgoon_01", "g_m_y_armgoon_02", "a_m_m_eastsa_02" });

            AddGangContact("triads", "Wei Cheng Contact", "TRI", "ecstasy", 0.70f, 1, 150f,
                "Pills by the bag. Cheap and constant.",
                new[] { "g_m_m_chiboss_01", "g_m_m_chigoon_01", "g_m_m_chigoon_02", "a_m_y_ktown_01" });

            AddGangContact("lost", "Lost MC Cook", "LOST", "meth", 0.70f, 1, 200f,
                "Cooks it himself out in Blaine County.",
                new[] { "g_m_y_lost_01", "g_m_y_lost_02", "g_m_y_lost_03", "a_m_m_hillbilly_01" });

            AddGangContact("vagos", "Vagos Connection", "VAGO", "coke", 0.70f, 2, 150f,
                "Cocaine, weight only, no small talk.",
                new[] { "g_m_y_mexgoon_01", "g_m_y_mexgoon_02", "g_m_y_mexgoon_03", "a_m_y_mexthug_01" });

            AddGangContact("families", "Families Plug", "FAM", "weed", 0.70f, 0, 200f,
                "Green by the pound. Always picks up.",
                new[] { "g_m_y_famca_01", "g_m_y_famdnf_01", "g_m_y_famfor_01", "a_m_y_soucent_01" });
        }

        private void AddGangContact(string gangId, string name, string tag, string drug,
                                    float priceMultiplier, int minRank, float maxOrder,
                                    string blurb, string[] models)
        {
            var def = new SupplierDef
            {
                Id = gangId + "_plug",
                Name = name,
                Tag = tag,
        ...
        /// <summary>Extra discount for being one of theirs.</summary>
        private const float AffiliatedDiscount = 0.90f;

        /// <summary>Extra discount for being well thought of, without being a member.</summary>
        private const float TrustedDiscount = 0.95f;
        private const float TrustedRepThreshold = 100f;

        /// <summary>Rep at or below this and a gang contact will not deal with you at all.</summary>
        private const float HatedRep = -50f;

        /// <summary>
        /// Why this contact will not take the call, or null if they will.
        ///
        /// Gang contacts care who you run with: a rival of your crew will not deal with you at
        /// any price, which is the cost side of picking a side.
        /// </summary>
        public string RefusalReason(SupplierDef def, PlayerState state, Affiliation crew)
        {
            ...
            if (!def.IsGangContact || crew == null) return null;

            var standing = crew.StandingFor(def.GangId);
            if (standing != null && standing.Rep <= HatedRep) return "They want you dead";

            if (crew.IsAffiliated)
            {
                var mine = crew.Current;
                if (mine.Id != def.GangId && (mine.IsRivalOf(def.GangId) ||
                    (crew.GangById(def.GangId)?.IsRivalOf(mine.Id) ?? false)))
                {
                    return "At war with " + mine.Name;
                }
            }
        ...
        /// <summary>
        /// Price multiplier after standing is taken into account. Running with a crew, or simply
        /// being well regarded by them, gets you a better number than a stranger.
        /// </summary>
        public float EffectiveMultiplier(SupplierDef def, Affiliation crew)
        {
            if (def == null) return 1f;

            var mult = def.PriceMultiplier;
            if (!def.IsGangContact || crew == null) return mult;

            if (crew.IsAffiliated && crew.Current.Id == def.GangId)
            {
                return mult * AffiliatedDiscount;
            }

            var standing = crew.StandingFor(def.GangId);
            if (standing != null && standing.Rep >= TrustedRepThreshold) return mult * TrustedDiscount;

            return mult;
        }

        /// <summary>Short note on why the price is what it is, for the wheel.</summary>
        public string PriceNote(SupplierDef def, Affiliation crew)
        {
            if (def == null || !def.IsGangContact || crew == null) return "";

            if (crew.IsAffiliated && crew.Current.Id == def.GangId) return "your crew -- 10% off";

            var standing = crew.StandingFor(def.GangId);
            if (standing != null && standing.Rep >= TrustedRepThreshold) return "trusted -- 5% off";
        ...
        public string ArrangeMeet(SupplierDef def, PlayerState state, Affiliation crew)
        {
            if (def == null) return "No such contact.";
            if (HasMeet) return "You already have a meet on -- " + _meetDef.Name + ".";

            var refusal = RefusalReason(def, state, crew);
            if (refusal != null) return def.Name + " will not take the call: " + refusal + ".";
