...
// C:\projects\parkview\src\Parkview\Core\Folk.cs. Change it there; the next build overwrites this.
using System;
...
namespace Hoodrich.Parkview.Core
{
    ...
    ///
    /// A STRAIGHT PORT of hoodrich Core/Folk.cs, and it must stay one: the decorator name,
    /// its type, Mark and Number are a contract with NPC Mind (Integration/Vouched), and a
    /// Parkview ped and a Hoodrich ped stood on the same mark are the SAME PERSON only if
    /// both mods turn that mark into the same number. Change it there, change it here.
    ///
